...
//Aca comienza para la Exportacion de los botones 
    ...
    //	"autoWidth": false,
        ...
        "columns":[
            ...
            {"data":null,
    			render: function (data, type, row ) {
    			    return "<button class='editar btn btn-primary  btn-sm' type='button' ><i class='fa fa-edit'></i><b></b></button>&nbsp;<button class='imprimir2 btn btn-danger  btn-sm' type='button' ><i class='fa fa-print'></i><b></b></button>&nbsp;<button class='foto btn btn-warning  btn-sm' type='button' ><i class='fa fa-image'></i><b></b></button>";
    			}
		    }
        ],
...
$('#tabla_clientes').on('click','.imprimir4',function(){
...
$('#tabla_clientes').on('click','.imprimir',function(){
    ...
    window.open("../MPDF/alumno_foto.php?codigo="+data.cliente_id);
})
...
$('#tabla_clientes').on('click','.imprimir2',function(){
    ...
    window.open("MPDF2/REPORTES/imprimir_carnet_qr_original.php?codigo="+data.cliente_id);
})



$('#tabla_clientes').on('click','.foto',function(){
  var data = table.row($(this).parents('tr')).data();//En tamaño escritorio
  if(table.row(this).child.isShown()){
      var data = table.row(this).data();
  }//Permite llevar los datos cuando es tamaño celular y usas el responsive de datatable
  $('#txt_foto_editar').val("");
  $('#txt_idcliente_foto').val("");
  $('#txt_nrodocumento_foto').val("");
  $('#txt_ruta_original').val("");
  $('#modal_editar_foto').modal({backdrop: 'static', keyboard: false})
  $("#modal_editar_foto").modal('show');
  if (data.cli_foto=="") {
    $("#div_imagen").html("<b>SIN FOTO</b>");
  }else{
    $("#div_imagen").html("<img src='clientes/"+data.cli_foto+"' with='50%' alt='Cargando foto...'>");
  }

  
  document.getElementById('txt_idcliente_foto').value=data.cliente_id;
  document.getElementById('txt_nrodocumento_foto').value=data.cli_nrodocumento;
  document.getElementById('txt_ruta_original').value=data.cli_foto;

})
function modificar_foto_cliente() {
    let idcliente = $('#txt_idcliente_foto').val();
    let txt_nrodocumento = $('#txt_nrodocumento_foto').val();
    let txtformato       = $('#txtformato').val();
    let txt_ruta_original= $('#txt_ruta_original').val();
    var form_data = new FormData();
    form_data.append("txt_foto_editar", $('#txt_foto_editar')[0].files[0]);
    form_data.append("idcliente", idcliente);
    form_data.append("nrodocumento", txt_nrodocumento);
    form_data.append("formato", txtformato);
    form_data.append("txt_ruta", txt_ruta_original);
    $.ajax({
        url:'../controller/clientes/controlador_editar_foto.php',
        type:'POST',
        data:form_data,
        contentType: false,
        processData: false,
    ...
    .done(function (resp) {
        if (resp>0) {
            Swal.fire("Mensaje de Confirmaci&oacute;n","<b>Imagen actualizada</b>","success");
            table.ajax.reload();
            $("#modal_editar_foto").modal('hide');
            $('#txt_foto_editar').val("");
        }else{
            Swal.fire("Mensaje de Error","No se pudo actualizar la imagen","error");
        }
    })
}
